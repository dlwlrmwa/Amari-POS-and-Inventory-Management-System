...
-- =====================================================
-- TRIGGER: Create audit log for sales void
-- =====================================================
CREATE OR REPLACE FUNCTION audit_sale_void()
RETURNS TRIGGER AS $$
DECLARE
    user_name VARCHAR(50);
BEGIN
    IF OLD.status != 'Voided' AND NEW.status = 'Voided' THEN
        SELECT username INTO user_name FROM users WHERE id = NEW.voided_by;
        ...
        INSERT INTO audit_logs (
            ...
            'Sale',
            NULL,
            'Sale voided: ' || NEW.salesid,
            'Status: ' || OLD.status,
            'Status: ' || NEW.status || ' | Reason: ' || COALESCE(NEW.void_reason, 'N/A')
        );
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS audit_sale_void ON sales;
CREATE TRIGGER audit_sale_void
    AFTER UPDATE ON sales
    FOR EACH ROW
    EXECUTE FUNCTION audit_sale_void();
